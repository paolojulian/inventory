...
func TestDeactiveProduct_ValidInput(t *testing.T) {
	...
	// == Start test ==
	req := httptest.NewRequest("POST", "/products/"+created.ID+"/de-activate", nil)
	...
	assert.Equal(t, false, updatedProduct.IsActive)
